...
# Alternate Colors
ALTERNATE_CORRECT_COLOR = "#6AAA64"  # Green for correct letters (alternate)
ALTERNATE_PRESENT_COLOR = "#C9B458"  # Yellow for misplaced letters (alternate)
CORRECT_COLOR = "#1338BE"  # blue for correct letters
PRESENT_COLOR = "#C35817"  # orange for misplaced letters


def toggle_colors():
    print("Toggling colors")
    global CORRECT_COLOR, PRESENT_COLOR
    if CORRECT_COLOR == "#1338BE":
        CORRECT_COLOR = ALTERNATE_CORRECT_COLOR
        PRESENT_COLOR = ALTERNATE_PRESENT_COLOR
    else:
        CORRECT_COLOR = "#1338BE"
        PRESENT_COLOR = "#C35817"
    # Update the game display here if necessary
    ...
    def __init__(self):
        ...
        def key_action(tke):
            ...
            if isinstance(tke, str):
                ...
                ch = tke.char.upper()
            if ch == "\x08" or ch == "\177" or ch == "DELETE":
                self.show_message("")
        ...
        toggle_button = tkinter.Button(
            root, text="Toggle Colors", command=toggle_colors
        )
        toggle_button.pack()
...
# ----- Integration Guide -----
# Ensure the following code is placed after the creation of the main Tkinter window (typically named 'root' or 'window')

# Create the toggle button
# toggle_button = tkinter.Button(window, text="Toggle Colors", command=toggle_colors)
# toggle_button.pack()
# ----------------------------
